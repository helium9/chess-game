...
const ChessBoard = () => {
  ...
  const handleCombineClick = (row, col) => {
    ...
    if (!combineAnchor) {
      ...
      if (!isEligible) {
        setMessage(
          announceError("Not eligible for combine. Select a highlighted piece.")
        );
        return;
      }
  ...
  const executeDeCombine = () => {
    ...
    const newGameState = {
      ...
      board: result.board,
      currentTurn:
        gameState.currentTurn === COLORS.WHITE ? COLORS.BLACK : COLORS.WHITE,
    };
      ...
      <div className="flex flex-col items-center max-w-6xl w-full relative z-10">
          ...
          <div className="flex flex-col items-center transform transition-transform hover:scale-105">
            ...
            <div className="min-h-16 flex flex-wrap gap-2 items-start justify-center w-36 bg-gradient-to-br from-slate-800 to-slate-900 p-3 rounded-xl border-2 border-amber-600/40 shadow-2xl backdrop-blur-sm">
              {gameState.capturedPieces.black.map((piece, idx) => (
                <span
                  key={idx}
                  className="text-3xl text-gray-900 transition-transform hover:scale-125"
              ...
              >
                {gameState.board.map((row, rowIndex) =>
                  row.map((piece, colIndex) => {
                    ...
                    const isCapture =
                      isLegalMove && gameState.board[rowIndex][colIndex];
                    ...
                    if (deCombine.mode) {
                      if (hybridSelected) {
                        ...
                        ringClass =
                          "ring-2 ring-green-400 ring-inset shadow-inner";
                        ...
                        ringClass =
                          "ring-2 ring-teal-300 ring-inset animate-pulse";
                      } else {
                      ...
                      if (anchor) {
                        ...
                        ringClass =
                          "ring-2 ring-purple-300 ring-inset shadow-inner";
                      } else if (eligible) {
                        squareColor = isLightSquare
                          ? "bg-gradient-to-br from-blue-200 to-blue-300"
                          : "bg-gradient-to-br from-blue-500 to-blue-700";
                        ringClass =
                          "ring-2 ring-blue-300 ring-inset animate-pulse";
                      }
                    } else {
          ...
          <div className="flex flex-col items-center transform transition-transform hover:scale-105">
            ...
            <div className="min-h-16 flex flex-wrap gap-2 items-start justify-center w-36 bg-gradient-to-br from-slate-800 to-slate-900 p-3 rounded-xl border-2 border-amber-600/40 shadow-2xl backdrop-blur-sm">
              {gameState.capturedPieces.white.map((piece, idx) => (
                <span
                  key={idx}
                  className="text-3xl text-white transition-transform hover:scale-125"
        ...
        <div className="mt-8 flex gap-4 flex-wrap justify-center">
          ...
          <button
            onClick={() => {
              ...
              if (newState !== gameState) {
                ...
                setLegalMoves([]);
                setMessage(
                  `Undo - ${capitalizeColor(newState.currentTurn)} to move`
                );
              }
            }}
          ...
          <button
            onClick={() => {
              ...
              if (newState !== gameState) {
                ...
                setLegalMoves([]);
                setMessage(
                  `Redo - ${capitalizeColor(newState.currentTurn)} to move`
                );
              }
            }}
          ...
          <button
            ...
            aria-label={
              combineMode ? "Cancel Combine Mode" : "Enter Combine Mode"
            }
          ...
          >
            <span className="text-xl">
              {deCombine.mode ? "❌ Cancel De-Combine" : "⚡ De-Combine Pieces"}
            </span>
          </button>
              ...
              <div className="text-center mb-4 space-y-2">
                <p className="text-lg">
                  ...
                  <span className="text-3xl">
                    {PIECE_SYMBOLS[deCombine.activeHybrid.piece]}
                  </span>
                </p>
                ...
                <p className="text-sm text-teal-300 italic">
                  Spawn square:{" "}
                  {String.fromCharCode(97 + deCombine.selectedSquare.col)}
                  {8 - deCombine.selectedSquare.row}
                </p>
              </div>
          ...
          >
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <button
                onClick={() =>
                  executePromotion(
                    gameState.currentTurn === "white" ? "Q" : "q"
                  )
                }
                autoFocus
              ...
              <button
                onClick={() =>
                  executePromotion(
                    gameState.currentTurn === "white" ? "R" : "r"
                  )
                }
              ...
              <button
                onClick={() =>
                  executePromotion(
                    gameState.currentTurn === "white" ? "B" : "b"
                  )
                }
              ...
              <button
                onClick={() =>
                  executePromotion(
                    gameState.currentTurn === "white" ? "N" : "n"
                  )
                }
