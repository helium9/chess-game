...
export const DE_COMBINE_ERRORS = {
    ...
    KING_IN_CHECK: 'De-combine here would expose your king; try a different square.',
    INVALID_ASSIGNMENT: 'Invalid component mapping.',
};

// Success message template
export const getSuccessMessage = (hybrid, stayingPiece, stayingSquare, spawningPiece, spawningSquare) => {
    const toAlgebraic = (row, col) => {
        return String.fromCharCode(97 + col) + (8 - row);
    };
    return `De-combined: ${hybrid} → ${getBasePieceType(stayingPiece)} at ${toAlgebraic(stayingSquare.row, stayingSquare.col)}, ${getBasePieceType(spawningPiece)} at ${toAlgebraic(spawningSquare.row, spawningSquare.col)}.`;
};
...
export const computeLegalAssignments = (board, row, col, spawnSquare, currentTurn) => {
    const piece = board[row][col];
    const components = getHybridComponents(piece);
    ...
    if (isLegalA) {
        legalAssignments.push({
            ...
            description: `${getBasePieceType(C1)} stays; ${getBasePieceType(C2)} spawns`
        });
    }
    ...
    if (isLegalB) {
        legalAssignments.push({
            ...
            stayingSquare: { row, col },
            spawningSquare: spawnSquare,
            description: `${getBasePieceType(C2)} stays; ${getBasePieceType(C1)} spawns`
        });
    }
...
export const validateDeCombineSafety = (board, row, col, spawnSquare, currentTurn) => {
    const result = computeLegalAssignments(board, row, col, spawnSquare, currentTurn);
    return result.legal.length > 0;
};

// Execute de-combination with chosen assignment (Spec 5.1)
export const executeDeCombination = (board, row, col, spawnSquare, currentTurn) => {
    const piece = board[row][col];
    const components = getHybridComponents(piece);
    ...
    const assignmentResult = computeLegalAssignments(board, row, col, spawnSquare, currentTurn);
    ...
    newBoard[chosen.stayingSquare.row][chosen.stayingSquare.col] = chosen.stayingComponent;
    newBoard[chosen.spawningSquare.row][chosen.spawningSquare.col] = chosen.spawningComponent;
...
export const findPlayerHybrids = (board, currentTurn) => {
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            const piece = board[row][col];
            if (piece && isHybridPiece(piece) && getPieceColor(piece) === currentTurn) {
...
export const validateDeCombination = (board, row, col, spawnSquare, currentTurn) => {
